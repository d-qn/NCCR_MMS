...
wcond5_labels_df <- wcond5_p_df %>%
  ...
  mutate(
    total_amelioration_lbl = paste0("<span style='color:#457651;'>", round(total_amelioration, num_n_dec), "%</span>"),
    total_deterioration_lbl = paste0("<span style='color:#985d45;'>", round(total_deterioration, num_n_dec), "%</span>"),
...
iwc_2 <- wcond3_p_df %>% 
 ...
 scale_y_continuous(
  ...
  annotate("text", x = nlevels(wcond3_p_df$country), y = 1,
           ...
           size = 3,
           family = mms_base_family, fontface = "bold",
           color = nccr_divergent_orange_green()[1]) #+debug_theme
...
iwc_3 <- wcond_sa_df %>% 
  ...
  facet_wrap(~varname, ncol = 1) +  # Remove scales = "free_y"
