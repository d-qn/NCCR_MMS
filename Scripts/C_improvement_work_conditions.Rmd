...
wcond_codes <- tibble(
  E27 = c(1, 2, 3, 4, 5),
  wcond5 = c("Amélioration substantielle", "Amélioration légère", 
            "Pas de changement", "Détérioration légère", 
            "Détérioration substantielle"),
...
data_all <- mms_df %>% 
  ...
  mutate(
    wcond5 = factor(wcond5, wcond_codes$wcond5),
    wcond3 = factor(wcond3, levels = c("Amélioration", "Identique", "Détérioration"), ordered = TRUE),
    # A2 year of birth
    age = year - A2,
    age_gr2 = ifelse(
...
wcond_area <- wcond5_df %>% 
  ggplot(aes(x = year, y = share, group = rev(wcond5))) +
  geom_area(aes(fill = wcond5), alpha = 0.9) + 
  theme_ipsum_rc(grid="", axis="xy", 
                 grid_col = "black",
