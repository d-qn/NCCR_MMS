...
iwc_3 <- wcond_sa_df %>% 
  ...
  geom_richtext(data = wcond_sa_labels, 
                aes(x = var, y = -0.07, label = total_amelioration_lbl),
                hjust = 1,  label.color = NA, size = 5#,
    #  fontface = "bold"
  ) +
  geom_richtext(data = wcond_sa_labels, 
                aes(x = var, y = 1.01, label = total_deterioration_lbl),
                hjust = 0, fill = NA, label.color = NA, size = 5) +
  geom_richtext(data = wcond_sa_labels,
                aes(x = var, y = y,
                    label = labels),
  ...
  theme(
    ...
    legend.position = "none",
    axis.text.y = element_blank(),
    panel.spacing.y = unit(0.7, "cm")  # reduce space between facets
  ) +
  scale_y_continuous(
    name = "", expand = c(0.07, 0.07),
    labels = function(x) paste0(x * 100, "%"),  # share est entre 0 et 1 ici
    breaks = c(0.5),
    position = "right"
  ) +
