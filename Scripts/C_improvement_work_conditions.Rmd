...
wcond3_p_df %<>% 
  left_join(pays2col) %>% 
  mutate(
    country = fct_reorder(country, share, max, .desc = F),
    color = ifelse(
      wcond3 == "Deteriorated", nccr_divergent_orange_green()[1],
      color),
    color = ifelse(
      wcond3 == "Unchanged", nccr_divergent_orange_green()[5],
      color)
  ) 
...
iwc_2 <- wcond3_p_df %>% 
  ...
  geom_bar(aes( fill = color, group = wcond3), stat = "identity", position = "fill") +
  coord_flip(clip = "off") +
  scale_fill_identity() +
...
iwc_3 <- wcond_sa_df %>% 
  ...
  theme(
    ...
    theme(aspect.ratio = 0.4)  # Makes height 50% of width
  ) +
  scale_y_continuous(
    name = "", expand = c(0.06, 0),
    labels = function(x) paste0(x * 100, "%"),  # share est entre 0 et 1 ici
    breaks = c(0.5),
    position = "right"
  ) +
