...
wcond5_labels_df <- wcond5_p_df %>%
  ...
  mutate(
    ...
    labels = gsub(" ", "<br>", as.character(pays))
  )
...
iwc_2 <- wcond5_p_df %>% 
 ...
 scale_y_continuous(
  ...
  scale_x_discrete(expand = c(0.05, 0), name = "") +
  labs(
    title = "L’amélioration de la situation professionnelle varie selon le pays d'origine",
    subtitle = "En 2024 avec la migration, la situation professionnelle a connu une: "
  ) +
  geom_richtext(
  ...
  annotate("text", x = nlevels(wcond5_p_df$pays), y = -.05, 
           label = "Amélioration", vjust = -5, hjust = 0.5, size = 4, 
           family = mms_base_family, fontface = "bold", 
           color = nccr_divergent_orange_green()[9]) +
  annotate("text", x = nlevels(wcond5_p_df$pays), y = 1.01, 
           label = "Détérioration", vjust = -5, hjust = 0.5, size = 4, 
           family = mms_base_family, fontface = "bold",
...
wcond_sa_df <- bind_rows(
  wcond_sexe_df %>% 
    ...
    mutate(varname = "Par sexe"),
  
  wcond_age_df %>% 
    rename(var = age_gr2, n_var = n_age) %>% 
    mutate(varname = "Par âge")  
) %>% 
  mutate(
    varname = fct_relevel(varname, "Par sexe", "Par âge")
  )
...
wcond_sa_labels <- wcond_sa_df %>% 
  ...
  summarise(
    total_amelioration = share[wcond3 == "Amélioration"],
    total_deterioration = share[wcond3 == "Détérioration"]
  ) %>% 
  mutate(
    total_amelioration_lbl = paste0("<span style='color:#457651;'>", round(total_amelioration, 1), "%</span>"),
    total_deterioration_lbl = paste0("<span style='color:#985d45;'>", round(total_deterioration, 1), "%</span>"),
...
iwc_3 <- wcond_sa_df %>% 
  ...
  geom_richtext(data = wcond_sa_labels, 
                aes(x = var, y = -0.02, label = total_amelioration_lbl),
                hjust = 1,  label.color = NA, size = 5#,
    #  fontface = "bold"
  ) +
  geom_richtext(data = wcond_sa_labels, 
                aes(x = var, y = 1.02, label = total_deterioration_lbl),
                hjust = 0, fill = NA, label.color = NA, size = 5) +
  geom_richtext(data = wcond_sa_labels,
                aes(x = var, y = y,
                    label = labels),
  ...
  theme(
    ...
    axis.text.y = element_blank(),
    panel.spacing.y = unit(0.2, "cm")  # reduce space between facets
  ) +
  scale_y_continuous(
  ...
  scale_x_discrete(expand = c(0.05, 0), name = "", drop = TRUE) +
  labs(
    title = "L’amélioration de la situation professionnelle varie selon le sexe et l'âge",
    subtitle = "En 2024 avec la migration, la situation professionnelle a connu une: "
  )
iwc_3
