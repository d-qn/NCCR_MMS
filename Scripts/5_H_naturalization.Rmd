...
p_nati_1 <- nati_y_df %>% 
  ...
  labs(
    title = str_c("The share of people ", 
                  "<span style='color:#54526B'>interested in naturalization</span> is gradually increasing"),
    subtitle="Do you plan to apply for Swiss naturalization in the future?") +
  scale_fill_manual(values = nccr_colors_5[c(6,1,10)])
...
p_nati_2 <- nati_migy_df %>% 
  ...
  geom_ribbon(
    ...
    alpha = 0.2, fill = nccr_colors_5[2]
  ) +
  ...
  scale_color_manual(values = nccr_colors_5[c(10,7)]) +
  scale_x_continuous(
  ...
  guides(color = "none") +
  labs(
    title = "A steady increase interest in naturalization over time spent in Switzerland",
    subtitle = str_c(
      "Share of people intending to naturalize, by survey year and by arrival cohorts (",
       "<span style='color:#54526B'><b>2006-2009</span>",
       " vs <span style='color:#948EB8'><b>2010-2014</span> cohort)")
  )
...
#nati_c_df$color <- nccr_colors_5[8]

nati_c_df %<>% 
  mutate(
    country_styled = str_c(
      "<span style='color:", "black", "'><b>", country, "</span>"
    ) %>% factor %>% fct_reorder(share, .fun = max, .desc = F)
  )
...
background_data <- nati_c_df %>% 
  select(-country_styled) # On retire la clé de facette
...
p_nati_3 <- nati_c_df %>% 
  ...
  geom_line(
    ...
    aes(group = country), # Important : grouper par l'ID original du pays
    color = nccr_colors_5[2],
    linewidth = 0.3, 
    alpha = 0.6
  ) +
  geom_line(alpha = 0.95, show.legend = FALSE, linewidth = 1, color = nccr_colors_5[7]) +
  geom_point(
    data  = fl_points, size = 2,
    color = nccr_colors_5[9]
  ) +
  ...
  #   inherit.aes = FALSE
  # ) +
  facet_wrap(~country_styled, ncol = 4, axes = "all_x") +
  theme_mms(grid=F, axis="x", 
            grid_col =  scales::alpha("lightgrey", 0.6),
  ...
  theme(
    ...
    strip.text = element_markdown(size = 9,
                                  hjust = 0.5,
                                  margin = margin(b = 0.5, t = 0.5))
  ) +
  # remove any scale expansion
  scale_x_continuous(expand = c(0.05,0.05), name = "") +
  # have y scale not start at 0, add unit % 
  scale_y_continuous(expand = c(0.025,0.025), name = "", 
                     labels = function(x) paste0(x, "%")) +
  labs(
...
p_nati_4 <- nati_cr_df %>% 
 ggplot() +
  ...
  scale_fill_manual(values = nccr_colors_5[c(9,6)]) +
  scale_y_continuous(expand = c(0,0), name = "", 
                     # limits = c(-85, 50),
                     breaks = NULL
  ...
  geom_richtext(
    data = nati_cr_labels,
    aes(x = country, y = c3_4, label = paste0(abs(round(c3_4, num_n_dect))),
        hjust = 0),
    fill = NA, label.color = NA,# remove background and outline,
  ...
  geom_richtext(
    data = nati_cr_labels,
    aes(x = country, y = c3_5, label = paste0(abs(round(c3_5, num_n_dect))),
        hjust = 1),
    fill = NA, label.color = NA,# remove background and outline,
    vjust = 0.5, nudge_x = 0, size = 2.5, alpha = 0.95, colour = "white",
    family = mms_base_family
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "5_H_naturalization.svg"),
  bg_col = pw_bg_color
  )
