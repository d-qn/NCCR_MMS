---
title: "More than half of migrant households in Switzerland have at least 3 members"
...
p_hh_1 <- hh_y_df %>% 
  ...
  #scale_fill_manual(values = nccr_categorical()[c(15, 3, 4)]) +
  #scale_fill_manual(values = c(nccr_bi_colors[15], nccr_colors_2[c(4,7)])) +
  scale_fill_manual(values = cos3_2) +
  labs(
    title = str_c("Over half of households have ", 
                    '<span style="color:', cos3_2[3], ';">three or more members'),
...
p_hh_2 <- hh_g_df %>% 
 ggplot(aes(x = sexe, y = share, fill = hh3)) +
  geom_bar(stat = "identity", position = "fill", linewidth = bar_width, color = "white") +
  geom_text(aes(y = share / 100, label = paste0(round(share, num_n_dect), ""),
  ...
  #coord_flip(clip = "on") +
  scale_fill_manual(values = cos3_2) + #c(nccr_bi_colors[15], nccr_colors_2[c(4,7)])) +
  ...
  scale_y_continuous(expand = c(0,0), name = "") +
   labs(
    title = str_c("Men are more likely to live ", 
                    '<span style="color:', cos3_2[1], ';">alone', "</span> than women"),
    subtitle = "Number of household members, by gender in 2024"
  ) +
  annotate(
    "text", x = 0.5, y = 1,
    angle = 90,
    label = "1 person", vjust = 0, hjust = 1, 
    size = 3.5,
  ...
  annotate(
    "text", x = 0.5, y = 0.7,
    angle = 90,
    label = "2 people", vjust = 0, hjust = 1, 
    size = 3.5,
    family = mms_base_family, fontface = "bold",
    color = cos3_2[2]
  ) +
  annotate(
    "text", x = 0.5, y = 0,
    angle = 90,
    label = "3+ people", vjust = 0, hjust = 0, 
    size = 3.5,
    family = mms_base_family, fontface = "bold",
    color = cos3_2[3]
  ) +
...
hh_value_labels <- hh2mekko_df %>% 
  ...
  mutate(
    x = xmin,
    y = ymax - 0.01,
    label = paste0(round(share, num_n_dec), ifelse(hh3 == "3+ people", "", "")),
    hjust = -0.05,
    color = ifelse(hh3 == "1 person", "black", "white")
  ) 
...
p_hh_3 <- hh2mekko_df %>%   
  ...
  scale_y_continuous(name = "",  labels = NULL, expand = c(0,0), limits = c(0,1.01))
...
p_hh_3a <- p_hh_3 +
  ...
  annotate("text", x = 0, y = 1,
           label = "1 person", vjust = -0.5, hjust = 0, 
           size = 3.5,
           family = mms_base_family, fontface = "bold",
           color = cos3_2[1]) +
  annotate("text", x = 27, y = 1,
           label = "2 people", vjust = -0.5, hjust = 0, 
           size = 3.5,
           family = mms_base_family, fontface = "bold",
           color = cos3_2[2]) +  
  annotate("text", x = 100, y = 1,
           label = "3+ people", vjust = -0.5, hjust = 1, 
           size = 3.5,
           family = mms_base_family, fontface = "bold",
           color = cos3_2[3]) +  
  coord_cartesian(clip = "off") +
  scale_color_identity() +
   labs(
     title = str_c("Higher proportion of people <span style='color:", cos3_2[1], ";'>living alone</span> among citizens of the European Union"),
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "2_G_household_size.svg"),
  bg_col = pw_bg_color
  )
