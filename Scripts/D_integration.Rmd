...
p_ints_1 <- ints_df %>% 
 ggplot(
  ...
  geom_label(
    x=3.7, aes(y=label_pos, label=label), size=2.5,
 ...
 # 
  ...
  labs(
    ...
    subtitle = str_c("Share of people who consider their education",
                     "<span style='color:", nccr_categorical()[11], "'><b> 'is suited' </b></span> vs. ", 
                     "<span style='color:", nccr_categorical()[7], "'><b> 'not suited'</b></span>",
    " to their job")
  ) +
...
p_ints_4 <- ints_sexe_df %>%
  ...
  geom_col(aes(y = 100), fill = nccr_bi_colors[15]) +
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "D_structural_integration.svg"),
  bg_col = pw_bg_color
  )
