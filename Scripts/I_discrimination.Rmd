...
p_dis_1 <- disy_df %>% 
 ...
 # scale_fill_manual(values = nccr_sequential_blue()[c(1,4,7,10)])
  scale_fill_manual(values = c(
    darken(nccr_bi_colors[10], 0.2), 
    darken(nccr_bi_colors[10], 0),
    lighten(nccr_bi_colors[9], 0), 
    nccr_bi_colors[9] %>% desaturate(0.4) %>% lighten(0.3)))
...
disrc_df %<>%
  mutate(
    share_label = ifelse(country == "Asia" & short_descr == levels(disrc_df$short_descr)[1],
                         paste0(share_label, "%"), share_label)
  )
