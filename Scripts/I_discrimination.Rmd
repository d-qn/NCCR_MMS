...
disrc_df %<>%
  mutate(
    ...
    share_label = round(weighted_share, 0)
  )

# Create the heatmap plot
disrc_df %>% 
ggplot(aes(x = short_descr, y = country, fill = weighted_share)) +
  
  # Add the heatmap tiles
  geom_tile(color = "white", linewidth = 0.7) +
  
  # Add the text labels inside the tiles
  geom_text(aes(label = share_label), color = "black", size = 3.5) +
  
  # Customize the color gradient
  scale_fill_gradient(low = "#e6f5ff", high = "#004c99", name = "Weighted Share (%)") +
  
  # Clean up the theme and labels
  labs(
    title = "Frequency of Discrimination by Situation and Country",
    x = "Situation",
    y = ""
  ) +
  theme_mms(
