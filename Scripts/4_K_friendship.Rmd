...
```{r 1}
cols3 <- c(nccr_colors_4[7], nccr_colors_4[1], nccr_colors_4[4])
...
p_fri_1 <- fri_df %>% 
  ...
  geom_area(
    aes(fill = g6), 
    alpha = 1, linewidth = bar_width, color = "white") +
  theme_mms(
    grid="XY", axis="XY", 
  ...
  labs(
    ...
    subtitle = str_c(
      ...
      "<span style='color:", nccr_colors_4[7], ";'><b>mostly in Switzerland</b></span>, ", 
       "<span style='color:", nccr_colors_4[2], ";'><b>mixed</b></span> or ",      
      "<span style='color:", nccr_colors_4[4], ";'><b>mostly abroad</b></span>, ",
      " by survey year"
    )) +
...
p_fri_2 <- frim_df %>% 
  ...
  labs(
    ...
    subtitle = str_c(
      ...
      # "<span style='color:", nccr_bi_colors[8], ";'>mostly in Switzerland</span>, ", 
      #  "<span style='color:", nccr_bi_colors[13], ";'>mixed</span> or ",      
      # "<span style='color:", nccr_bi_colors[4], ";'>mostly abroad</span>"
    )
  ) +
  geom_text(data = frim_df %>% filter(g6 == g62f$g6[1]), 
            aes(y = 100 - share, label = paste0(round(share, num_n_dec))), 
            vjust = 0, nudge_y = 2,
            size = 3.5, color = "white", family = mms_base_family) +
  geom_text(data = frim_df %>% filter(g6 == g62f$g6[5]),
            aes(y = share, label = paste0(round(share, num_n_dec))), 
            vjust = 1, nudge_y = -2,
            size = 3.5, color = "white", family = mms_base_family) +
   annotate(
    "text", x = 0.5, y = 100,
    angle = 90,
    label = "Mostly in\nSwitzerland", vjust = 0, hjust = 1, 
    size = 3.2,
...
p_fri_3 <- fric_df %>% 
  ...
  geom_segment(data = fric_2_df, 
               ...
               color = cols3[3], size = segm_lw, alpha = 0.8) +
  ...
  labs(
    ...
    subtitle = str_c(
      ...
      "<span style='color:", cols3[1], ";'><b>mostly in Switzerland</b></span>, ", 
       "<span style='color:", cols3[2], ";'><b>mixed</b></span> or ",      
      "<span style='color:", cols3[3], ";'><b>mostly abroad</b></span>"
    )
  ) +
  geom_text(data = fric_1_df, 
            ...
            size = val_txt, color = cols3[1], family = mms_base_family) +
  geom_text(data = fric_2_df,
            aes(x = share, y = country, 
                label = paste0(round(label_share, num_n_dect))), 
            vjust = 0, hjust = 0.5, nudge_y = nudge_y_txt,
            size = val_txt, color = cols3[3], family = mms_base_family)
...
p_fri_4 <- frimg_df %>% 
  ...
  geom_bar(aes(fill = g20), stat = "identity", 
           position = "stack", alpha = 1, linewidth = bar_width, 
           color = "white") +
  theme_mms(
    grid=F, axis=F, 
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 5),
  ...
  scale_fill_manual(values = nccr_colors_4[c(2,6)]) +
  labs(
    ...
    subtitle = str_c(
      "Presence, ",
      "<span style='color:", nccr_colors_4[7], ";'><b>yes</b></span> or ",
      "<span style='color:", nccr_colors_4[3], ";'><b>no</b></span>, ",
      "of a person to count on in Switzerland in 2024, by year of migration"
    )
  ) +
  geom_text(data = frimg_y_labels, 
            aes(y = share, label = paste0(round(share, num_n_dect))), 
            vjust = 1, nudge_y = -3,
            size = 3, color = "white", family = mms_base_family) +
  geom_text(data = frimg_n_labels,
            aes(y = 100 - share, label = paste0(round(share, num_n_dect))), 
            vjust = 0, nudge_y = 3,
            size = 3, color = "white", family = mms_base_family)
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "4_K_friendship.svg"),
  bg_col = pw_bg_color
  )
