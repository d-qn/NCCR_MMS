...
edu_chart_1 <- edu_main %>% 
  ...
  labs(
    title = str_c("Strong increase in ", 
                  "<span style='color:", nccr_colors_2[7],
                  "'>highly skilled</span> migration"),
    subtitle="Highest level of education attained, by survey year") +
  scale_fill_manual(values = nccr_colors_2[c(2, 4, 7)])


annot_df <- tibble(
  year = rep(2020, 3),
  share_w = c(27, 72, 93),
  txt = c("Tertiary", "Secondary II", "Secondary I")
)
...
edu_chart2 <- edu_all_genre %>% 
  ...
  #   aes(ymin = men, ymax = women, group = 1),
  #   fill = nccr_bi_colors[13], alpha = 0.15) +
  geom_point(data = edu_all_genre %>% filter(type == "total"), 
             aes(x = year, y = share_w, 
                 group = type,
                 color = type), 
             shape = 15, alpha = 1, size = 3) +
  geom_line(data = edu_all_genre %>% filter(type == "total"), 
            aes(x = year, y = share_w, group = type,
                color = type), alpha = 0.2, 
            linewidth = 2,
  ...
  scale_y_continuous(
    ...
    position = "right") +
  scale_colour_manual(
    values=c(nccr_colors_2[c(5,2,7)])) +
...
share_tot_3 <- edu_all_genre %>% 
  filter(year %in% c(2018, 2024) & type == "total") %>% 
  select(year, share_w) %>% 
  mutate(type = "Moyenne")
...
edu3_pays_df <- edu3_pays %>%
  ...
  group_by(type) %>%
  mutate(
    dir = ifelse(share_w[2] >= share_w[1], "up", "down")
  ) %>% 
  ungroup()
...
edu_chart3 <- edu3_pays_df %>% 
  ...
  geom_point(
    data = edu3_pays_df %>%
      filter(year == 2024),
    aes(group = year, shape = dir),
    fill = nccr_colors_2[7],
    color = nccr_colors_2[7],
    alpha = 1, show.legend = F, #shape = 17,
    size = pt_size) +
  ...
  theme(
    ...
    legend.title = element_text(
      ...
      face = "bold", hjust =0.5), # Red◾️uce legend title size
  ...
  labs(
    ...
    subtitle = str_c("Change in tertiary education rate between 2018 ", #(&#9726;)&nbsp;",
                     
                      "and 2024, by country of origin"
  )) + 
...
# patchwork 
#   grid.draw(grid.polygon(
#   x = unit(c(0.46, 0.53, 0.53), "npc"),      # x-coordinates of the vertices
#   y = unit(c(0.76, 0.87, 0.53), "npc"),    # y-coordinates of the vertices
#   gp = gpar(fill = nccr_bi_colors[15], 
#             col =nccr_bi_colors[14], alpha = 0.1) # customize appearance
# ))

mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "2_A_education.svg"),
  bg_col = pw_bg_color
  )
