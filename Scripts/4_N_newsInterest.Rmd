...
cols3 <- c(nccr_colors_4[2], nccr_colors_4[4], nccr_colors_4[7])
...
nih_cols <- cols3  %>% desaturate(0.3) %>% lighten(0.1)
#nih_cols[1] <- lighten(nih_cols[1], 0.4) %>% desaturate(0.5)

box_col <- cols3[2] %>% desaturate(0.1) %>% lighten(0.4)
box_coli <- box_col %>% desaturate(0.4) %>% lighten(0.1)
bg_coli <- box_coli %>% desaturate(0.6) %>% lighten(0.6)
...
p_ni_1a <- nic_df %>% 
 ...
 p_ni_1b <- nih_df %>% 
   ...
   theme(
     legend.position = "none",
     # panel.background = element_rect(fill = bg_coli),  # transparent
     # plot.background = element_rect(fill = bg_coli),
  ...
  scale_fill_manual(values = nih_cols)
...
patchwork <- plot_final +
  ...
  plot_annotation(
    ...
    subtitle = str_c("Level of interest: ",
                    "<span style='color:", ni_cols[1] %>% darken(0.1),
                    ";'><b>low</b></span>, ",
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "4_N_newsInterest.svg"),
  bg_col = pw_bg_color
  )
