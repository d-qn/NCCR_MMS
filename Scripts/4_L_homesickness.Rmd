...
cols3 <- c(nccr_colors_4[1], nccr_colors_4[4], nccr_colors_4[7])
```
 ...
 ggplot(
  ...
  labs(
    ...
    subtitle = str_c("Reported homesickness in 2024, by gender: ",
                   "<span style='color:", darken(cols3[1], 0.2) , ";'><b>weak</span></b>, ",
                   "<span style='color:", cols3[2], ";'><b>moderate</span></b>",
                   " or <span style='color:", darken(cols3[3], 0.2), ";'><b>strong</span></b> "
                     )
  ) +
...
p_ho_2 <- ho_m_df %>% 
  ggplot(aes(x = mig_4y, y = share, fill = rev(g26))) +
  geom_col(position = "fill", color = "white", linewidth = bar_width) +
  theme_mms(
    grid=F, axis=F,
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 4),
    mms_plot_subtitle = mms_txtbox_plot.subtitle(nrow_txt = 4)
  ...
  scale_fill_manual(values = rev(cols3)) +
  labs(
  ...
  scale_color_manual(values = c("black" = darken(cols3[3], 0.3), 
                                "white" = "white"))
...
ho_c_labels <- ho_c_df %>%
  ...
  mutate(
    weak_lbl = paste0("<span style='color:", darken(cols3[3], 0.4), ";'>", round(weak, num_n_dect), "</span>"),
...
p_ho_3 <- ho_c_df %>% 
  ...
  geom_col(aes( fill = rev(g26), group = rev(g26)), 
           position = "fill", color = "white", linewidth = bar_width) +
  coord_flip(clip = "off") +
  theme_mms(
    grid="X", axis=F,
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 4),
 ...
 scale_y_continuous(
  ...
  geom_richtext(
    data = ho_c_labels,
    aes(x = country, y = 1.008, label = strong_lbl),
    family = mms_base_family,
...
patchwork <- (left_col | p_ho_3) + plot_layout(widths = c(1, 1))
patchwork

mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "4_L_homesickness.svg"),
  bg_col = pw_bg_color
  )
