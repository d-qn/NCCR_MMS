---
title: "Migration generally leads to an improvement in working conditions"
...
wcond3_df <- c_df %>% 
  ...
  mutate(
    ...
    share = weight/sum(weight, na.rm = TRUE),
  ) %>% 
  ungroup()
...
iwc_1 <- wcond3_df %>% 
 # arrange(wcond5) %>%
  ggplot(aes(x = year, y = share, group = wcond3)) +
  geom_area(aes(fill = wcond3), alpha = 1, 
  ...
  scale_y_continuous(expand = c(0,0), name = "", 
                     ...
                     breaks = c(25,50,75))+
  scale_fill_manual(values = nccr_colors_3[c(3,4,1,8,9)]) +
  labs(
    title = 
      str_c(
...
imp_col <- nccr_colors_3[9]
det_col <- nccr_colors_3[3]
cent_col <- nccr_colors_3[1]
...
wcond3_labels_df <- wcond3_p_df %>%
  ...
  mutate(
    total_amelioration_lbl = paste0("<span style='color:white;'>", round(total_amelioration, num_n_dec), "</span>"),
    total_deterioration_lbl = paste0("<span style='color:black;'>", round(total_deterioration, num_n_dec), "</span>"),
...
iwc_2 <- wcond3_p_df %>% 
  ...
  geom_bar(aes( fill = wcond3), stat = "identity", position = "fill", linewidth = bar_width, color = "white") +
  coord_flip(clip = "off") +
  scale_fill_manual(values = c(det_col, cent_col, imp_col)) +
  theme_mms(
    grid="X", axis=F,
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 4),
    mms_plot_subtitle = mms_txtbox_plot.subtitle(nrow_txt = 2)
 ...
 scale_y_continuous(
  ...
  geom_richtext(
    ...
    fill = NA,
    family = mms_base_family,
    hjust = 1, label.color = NA, size = 2.5
  ) +
  geom_richtext(
    data = wcond3_labels_df,
    aes(x = country, y = 1.005, label = total_deterioration_lbl),
    family = mms_base_family,
    hjust = 1, fill = NA, label.color = NA, size = 2.5
  ) +
  geom_richtext(
    data = wcond3_labels_df,
    aes(x = country, y = y,
...
wcond_sa_labels <- wcond_sa_df %>% 
  ...
  mutate(
    total_amelioration_lbl = paste0("<span style='color:#f2f2f2;'>", round(total_amelioration, num_n_dec), "</span>"),
    total_deterioration_lbl = paste0("<span style='color:#ede1de;'>", round(total_deterioration, num_n_dec), "</span>"),
...
iwc_3 <- wcond_sa_df %>% 
  ...
  # Replace facet_wrap with facet_grid
  facet_wrap(~varname, ncol = 1, scales = "free_y") +  # Remove scales = "free_y"
  #facet_grid(rows = vars(varname), scales = "free_y", space = "free_y", switch = "y") + 
  ...
  theme(
    ...
    panel.spacing.y = unit(15, "pt"),  # reduce space between facets
    strip.text = element_text(angle = 0, hjust = 0),# 0 = left, 0.5 = center, 1 = right
    theme(aspect.ratio = 0.5),  # Makes height 50% of width
    #strip.text = element_blank(),  # Hide facet labels
    strip.background = element_blank(),
    #strip.placement = "outside",  # Place strip outside of axis
   # strip.text.y.left = element_text(angle = 0, hjust = 0)  # Keep text horizontal
  ) +
  scale_y_continuous(
    name = "", expand = c(0, 0),
...
iwc_4 <- wcond_mig4y_df %>% 
  ...
  geom_col(aes(fill = wcond3), linewidth = bar_width, color = "white") + 
  geom_text(
    data = wcondi_labels,
...
patchwork <- ((iwc_1a /iwc_4) +
  plot_layout(heights = c(1.5, 1.3)) | iwc_2) +
  plot_layout(widths = c(1,1))
patchwork

patchwork <- ((iwc_1a /iwc_4) +
  plot_layout(heights = c(3,1), widths = 1, ncol = 1, guides = "collect") | (iwc_2/iwc_3) +
