...
annot_df <- tibble(
  ...
  share_w = rev(c(40,  86, 96)),
  txt = wcond3_df$wcond3 %>% levels(),
  hjust = rep(0.5, 3)
) %>% 
  left_join(
    wcond3_df %>% group_by(wcond3) %>% 
...
wcond3_labels_df <- wcond3_p_df %>%
  ...
  mutate(
    ...
    total_deterioration_lbl = paste0("<span style='color:#364449;'>", round(total_deterioration, num_n_dec), "</span>"),
...
iwc_2 <- wcond3_p_df %>% 
 ...
 scale_y_continuous(
  ...
  geom_richtext(
    data = wcond3_labels_df,
    aes(x = country, y = 1, label = total_deterioration_lbl),
    family = mms_base_family,
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "3_C_work_improvement.svg"),
  bg_col = pw_bg_color
  )
