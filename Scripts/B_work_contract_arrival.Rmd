...
mig_job_1 <- mig4y_df %>% 
  ...
  scale_y_continuous(expand = c(0.017,0.012), name = "", 
...
mig_job_2 <- fhf_df %>% 
  ...
  scale_x_discrete(expand = c(0.4,0.1))+
    scale_y_continuous(expand = c(0,1), name = "", 
                     labels = function(x) paste0(x, "%")) +
  scale_fill_manual(values = nccr_bi_colors[c(1,3)]) +
  scale_color_manual(values = nccr_bi_colors[c(1,3)]) +
   labs(
    x = "", y = "",
    ...
    nccr_bi_colors[3], ";'>hommes</span> que chez les <span style='color:",
                  nccr_bi_colors[1], 
...
col_seul <- nccr_bi_colors[7] #"#c9d175" 
...
mig_job_3 <- fpp_chart +
  ...
  labs(
    title = str_c("Les ressortissants de pays voisins s’installent plus souvent en Suisse avec un emploi — sauf les Italiens"),
