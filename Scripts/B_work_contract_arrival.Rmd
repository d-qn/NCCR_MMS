...
mig_job_1 <- mig4y_df %>% 
  ...
  labs(
    ...
    title = "Près d’une personne sur deux arrive en Suisse avec un contrat de travail<br><br>",
    size = "n",
    linewidth = "n"
  )
  ...
  scale_color_manual(values = nccr_bi_colors[c(2,4)]) +
   labs(
    ...
    nccr_bi_colors[4], ";'>hommes</span> que chez les <span style='color:",
                  nccr_bi_colors[2], 
                  ";'>femmes</span><br>")# à l’arrivée en Suisse")
  )
...
fpp2mekko_df <- fpp2mekko_df %>% 
  ...
  mutate(
    fill = ifelse(
      pays %in% c("France", "Allemagne", "Autriche", "Italie") &
        contrat == "Oui", 
...
mig_job_3 <- fpp_chart +
  ...
  labs(
    title = str_c("Les ressortissants de <span style='color:",
                  col_voisin, 
                  ";'>pays voisins</span> s’installent plus souvent en Suisse avec un emploi"),
