...
p_hc_1 <- hc_df %>% 
  ...
  scale_fill_manual(values = nccr_categorical()[c(13, 10)]) +
  labs(
...
p_hc_2 <- hcv_df %>% 
  ...
  scale_y_continuous(expand = c(0,0), name = "", 
                     breaks = seq(25, 100, 25),
                     labels = function(x) paste0(x, "%")) +
  scale_fill_manual(values = nccr_categorical()[c(13, 9)]) +
  labs(
...
p_hc_3 <- hccp_df %>%   
  ggplot(aes(x = share, y = country, fill = color)) +
  ...
  geom_text(
    aes(label = paste0(round(share, 1), "%"), x = share),
    color = "white",
    size = 3,
    hjust = 1.1,
    family = mms_base_family
