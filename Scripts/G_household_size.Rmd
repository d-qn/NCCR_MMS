...
p_hh_2 <- hh_g_df %>% 
 ggplot(aes(x = sexe, y = share, fill = hh3)) +
  ...
  geom_text(aes(y = share / 100, label = paste0(round(share), "%")), size = 3,
            position = position_stack(vjust = 0.85), family =mms_base_family,
            color = "white") +
...
p_hh_3a <- p_hh_3 +
  ...
  geom_richtext(
    ...
    aes(x = x + 0.01, y = ymin, label = label, hjust = hjust, color = color),
    fill = NA, label.color = NA,# remove background and outline,
    vjust = 0, nudge_y = 0.005, size = 3, alpha = 0.95,
    family = mms_base_family
  ) +
  scale_color_identity() +
   labs(
     ...
     caption = str_c("Bar height is proportional to sample size (n), total respondents: ",#,
                     hh2mekko_df$n %>% sum()%>% prettyNum(big.mark = " ")
   )
   )
