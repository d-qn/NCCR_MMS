...
sa_cols <-  nccr_colors_6[c(2,5,8)]
#sa_cols[2] <- sa_cols[2] %>% desaturate(0.1)
#sa_cols[1] <- lighten(sa_cols[1], 0.5) %>% desaturate(0.5)
...
p_sa_1 <- sa_df %>%
  ...
  geom_text(
    ...
    aes(x = year, y = share_w, label = str_to_title(label)),
    color = "white",
    size = 5,
...
gender_labels <- sa_s_df %>%
  distinct(sexe) %>%
  mutate(
    # On transforme la valeur de la colonne sexe (ex: "men") en label (ex: "Men")
    # Si tu veux traduire, utilise case_match ou recode, mais base-toi sur la colonne 'sexe'
    label = sexe
  )
...
p_sa_2 <- sa_s_df %>%
 ggplot(
  ...
  geom_label(
    ...
    x=3.7,
    aes(y=label_pos, label=label, group = h6),
    color = "white",
    size = 4,
  ...
  geom_text(
    data = gender_labels,
    aes(x = 2, y = 0, label = label), # Position text at the center
  ...
  labs(
    title = str_c(
                  "Women are less satisfied than ",
                  "men with their move to Switzerland"),
...
p_sa_3 <- sa_n_df %>%
  ...
  labs(
    title = str_c("People intending to naturalize and with higher education are more satisfied with their move to Switzerland"),
    subtitle = str_c("Level of satisfaction of moving to Switzerland, in 2024 (",
                   "<span style='color:", sa_cols[3],
                    ";'><b>high</b></span>, ",
                   "<span style='color:", sa_cols[2], ";'><b>moderate</b></span> or ",
                   "<span style='color:", sa_cols[1], ";'><b>low</b></span>), ",
                   "by intent to naturalize: yes, undecided or no" )
  )
...
o_countries <- sa_ec_df %>% 
  group_by(country) %>%
  reframe(mshare = max(share)) %>% 
  arrange(mshare) %>% 
  .$country

# a dot plot range chart by country & edu ?? 
sa_ecw <- sa_ec_df %>% 
  select(country, edu, share) %>% 
  pivot_wider(
    names_from = edu,
    values_from = share
  ) %>% 
  mutate(
    country = factor(country, levels = o_countries),
    largert = ifelse(`Secondary I & II` > Tertiary, TRUE, FALSE),
    minv = pmin(`Secondary I & II`, Tertiary),
  ) %>% 
  arrange(country)
...
annot_sub <- sa_ec_df %>% 
  filter(country == "Other Europe") %>% 
  select(edu, country, share) %>% 
  mutate(country = factor(country, levels = levels(sa_ecw$country)))

# A tibble: 2 × 5
arrow_txt <-
  tibble(
    x2 = annot_sub$share+c(-0.2,0.1),
    x1 = annot_sub$share,
    y2 = as.numeric(annot_sub$country)+c(0.7, -1),
    y1 = as.numeric(annot_sub$country),
    label = c(
      str_c("Share of people<br>with high level of<br>satifaction with<br><b>", 
            "secondary I & II",
            "<br>education"),
      str_c("with <br><b>", "tertiary", "<br>education")),
    hj = c(1,0),
    vj = c(0,1)
  )
...
p_sa_5 <- sa_ecw %>% 
  ...
  geom_curve(
    ...
    alpha = 0.7,
    curvature = 0.2, size = .4) +
  geom_richtext(
    data = arrow_txt,
    aes(x = x2, y = y2, label = label, hjust = hj, vjust = vj),
    family = mms_base_family,
    #fontface ="bold",
  ...
  geom_segment(aes(x = `Secondary I & II`, xend = Tertiary, 
                   y = country, yend = country,
                   color = largert),
               size = 1, alpha = 0.7) +
  geom_point(aes(x = `Secondary I & II`, y = country), 
             color = sa_cols[3], size = point_s, 
             shape = 20)  +
  geom_point(aes(x = Tertiary, y = country), 
             color = sa_cols[3], size = point_s,
             shape = 18) +
  scale_color_manual(values = c("TRUE" =  nccr_colors_6[2] %>% 
                                  clr_lighten(0.3) %>% clr_desaturate(0.6), 
                                "FALSE" =  nccr_colors_6[6])) +
  theme_mms(
    grid="X", axis="X",
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 4),
    mms_plot_subtitle = mms_txtbox_plot.subtitle(nrow_txt = 4)
  ...
  scale_y_discrete(name = "", expand = c(0.1, 0.015)) +
  theme(
    legend.position = "none",
    axis.text.y = element_blank(),
  ) +
  geom_text(
    aes(x = minv, y = country, label = country),
    size = 3.5,
    hjust = 1.1,
    vjust = 0.5,
    nudge_x = -0.3,
  ...
  labs(
    title = str_c("People originating from Southern Europe are generally less satisfied with having moved to Switzerland"),
    subtitle = str_c(
      "Proportion of people with a high level of satisfaction according to their education level",
      " (● secondary I & II or ◆ tertiary)",
      ", in 2024 and by country of origin")
  )
...
top_p <- (p_sa_1 | (p_sa_3 / p_sa_4)) + plot_layout(widths = c(1, 1.2))
bottom_p <- (p_sa_2 |p_sa_5) + plot_layout(widths = c(1, 1.2))

patchwork <- (top_p / bottom_p) +
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "6_O_satisfaction.svg"),
  bg_col = pw_bg_color
  )
