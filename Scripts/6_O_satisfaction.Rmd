...
sa_cols <- nccr_bi_colors[c(1,1,2)]
...
p_sa_2 <- sa_s_df %>%
 ggplot(
  ...
  labs(
    title = str_c("<span style='color:", nccr_bi_colors[12],  ";'><b>",
                  "Women</b></span>", " are less satisfied than ",
                  "<span style='color:", nccr_bi_colors[2],  ";'><b>",
                  "men</b></span> with their move to Switzerland"),
...
sa_ecw <- sa_ec_df %>% 
  ...
  mutate(
    country = factor(country) %>% fct_reorder(Tertiary),
...
arrow_txt <-
  tibble(
    x2 = annot_sub$share+c(-1,1),
    x1 = annot_sub$share,
    y2 = as.numeric(annot_sub$country)+c(0.7, 0.05),
...
p_sa_5 <- sa_ecw %>% 
  ...
  geom_richtext(
    data = arrow_txt,
    aes(x = x2, y = y2, label = label, hjust = hj),
    family = mms_base_family,
    fontface ="bold",
  ...
  geom_segment(aes(x = `Secondary I & II`, xend = Tertiary, 
                   ...
                   color = largert),
               size = 1.5, alpha = 0.8) +
  geom_point(aes(x = `Secondary I & II`, y = country), color = sa_cols[2], size = point_s, 
             shape = 15)  +
  geom_point(aes(x = Tertiary, y = country), color = sa_cols[3], size = point_s) +
  scale_color_manual(values = c("TRUE" =  nccr_bi_colors[13], "FALSE" =  sa_cols[1])) +
  theme_mms(
    grid="X", axis="X",
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 4),
    mms_plot_subtitle = mms_txtbox_plot.subtitle(nrow_txt = 4)
  ...
  labs(
    ...
    subtitle = str_c("Proportion of people with a high level of satisfaction according to their education level",
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "O_satisfaction.svg"),
  bg_col = pw_bg_color
  )
