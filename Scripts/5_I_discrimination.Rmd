...
p_dis_1 <- disy_df %>% 
 ...
 # scale_fill_manual(values = nccr_sequential_blue()[c(1,4,7,10)])
  scale_fill_manual(values = nccr_colors_5[c(10,8,5,3)])

 annot_df <- tibble(
   year = rep(2021, 4),
   share_w = c(27, 62, 86, 98.1),
   txt = c("Never", "Rarely", "Sometimes", "Frequently")
 ) %>% 
   left_join(
      disy_df %>% group_by(h7) %>% reframe(m_share = mean(share)),
      by = c("txt" = "h7")
   )
p_dis_1a <- p_dis_1  +
   geom_richtext(data = annot_df, aes(
     x = year, y = share_w, label = txt, size = m_share, group = 1),
...
p_dis_2 <- disr_df %>% 
  ...
  geom_point(
    color = nccr_colors_5[10], # Using one of your nccr_bi_colors
    size = 3,
    alpha = 1
  ) +
  # Add the numeric value as a text label next to the dot
  geom_text(
    aes(label = paste0(round(weighted_share, num_n_dect))),
    hjust = -0.37, # Position the text slightly to the right of the point
    size = 3,
    nudge_x = 0.2,
    color = nccr_colors_5[3] %>% clr_darken(0.5), 
...
disrc_df %<>%
  mutate(
    ...
    share_label = round(weighted_share, num_n_dec),
    text_color = "white" #if_else(weighted_share >= midpoint, "white", "black")
 )
...
p_dis_3 <- disrc_df %>% 
  ...
  scale_fill_gradient(low = nccr_colors_5[2], #%>% clr_desaturate (0.1) %>% clr_darken(0.2), 
                      high = clr_saturate(nccr_colors_5[10], 0.1) %>% clr_darken(0.2), 
                      name = "Weighted Share (%)") +
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "5_I_discrimination.svg"),
  bg_col = pw_bg_color
  )
```
