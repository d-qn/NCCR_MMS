...
p_hc_1 <- hc_df %>% 
  ...
  geom_line(data = hc_df %>% filter(g7 == "Yes"),
            color = "white", linewidth = bar_width
  ) +
  theme_mms(
    grid="XY", axis="xy", 
...
p_hc_3 <- hccp_df %>%   
  ...
  geom_col(aes(y = country, x = 100), 
           fill = nccr_colors_4[1], linewidth = bar_width) +
  geom_col(aes(x = share, y = country),
           show.legend = FALSE, linewidth = bar_width,
           color = "white", fill = nccr_colors_4[6]) +
  theme_mms(
    grid=F, axis=F, 
    mms_plot_title = mms_txtbox_plot.title(nrow_txt = 3),
    mms_plot_subtitle = mms_txtbox_plot.subtitle(nrow_txt = 2)
  ...
  geom_text(
    aes(y = country, 
        label = paste0(round(share, num_n_dec)), x = share),
    color = "white",
    size = 3,
    hjust = 1.1,
    vjust = 1,
...
mms_export_vector(
  plot = patchwork,
  filepath = here("Output", "charts", "4_M_homeCountryVisit.svg"),
  bg_col = pw_bg_color
  )
