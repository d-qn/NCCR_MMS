...
p_nati_3 <- nati_c_df %>% 
  ...
  facet_wrap(~country_styled, ncol = 3) +
  ...
  # remove any scale expansion
  scale_x_continuous(expand = c(0.05,0.05), name = "", breaks = c(2016, 2024)) +
  # have y scale not start at 0, add unit % 
  scale_y_continuous(expand = c(0.02,0.02), name = "", 
                     labels = function(x) paste0(x, "%")) +
  labs(
    title = "Interest in naturalization varies by country of origin",
    subtitle = "Share of people intending to naturalize, by country of origin"
  ) +
 ...
 ggplot() +
  ...
  geom_richtext(
    ...
    aes(x = country, y = c3_4, label = paste0(abs(round(c3_4)), "%"),
        hjust = 0),
    fill = NA, label.color = NA,# remove background and outline,
    vjust = 0, nudge_x = 0, size = 3, alpha = 0.95, colour = "white",
    family = mms_base_family
  ) +
  geom_richtext(
    data = nati_cr_labels,
  ...
  labs(
    ...
    subtitle = str_c("Proportion of people citing 'a sense of belonging' versus 'improved professional opportunities' as reasons for seeking naturalization, 2024 (multiple responses allowed)<br>")
  )
