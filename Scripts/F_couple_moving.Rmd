...
b102f <- tibble(
  ...
  mutate(
    b10 = factor(b10, levels = b102f$b10[c(2,1, 3, 4, 5)])
  )
...
p_coum_2a <-  p_coum_2 +
   ...
   geom_richtext(
    ...
    annotate(
    geom = "richtext",
    x=92,y = 0.41, angle = -90, label = "alone",
    fill = NA, label.color = NA,# remove background and outline,
    size = 8,
  ...
  labs(
    ...
    caption = str_c("<br>Bar height is proportional to sample size (n), total respondents: ",coum2mekko_df$n %>% sum()%>% prettyNum(big.mark = " "))
  )
...
cou_msa_df %<>% 
  mutate(
    ...
    label = str_c(b10, " <span style='font-size:12px;'>", share, "%</span>"),
    color = nccr_bi_colors[c(10,12,6,8,14)]
  )
...
p_coum_3 <- cou_msa_df %>% 
  ...
  scale_fill_manual(values = nccr_bi_colors[c(9,11,5,7,13)])  +
