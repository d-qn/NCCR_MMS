...
p_coum_2a <- p_coum_2 +
  ...
  geom_richtext(
    ...
    nudge_y = 0.005, # (Garde ton nudge original)
    size = 3, 
    alpha = 1, 
    colour = "white",
    family = mms_base_family
  ...
  annotate(
    geom = "richtext",
    x = 90, 
    # TRADUCTION : y=0.51 (continu) devient le milieu de l'axe discret
    y = (length(coum_p_ordered) + 1) / 2, 
    angle = -90, 
    label = "alone",
    fill = NA, label.color = NA,
    size = 13,
...
cou_msa_df %<>% 
  mutate(
    ...
    color = nccr_colors_1[c(8,6,5,4,3)],
    label = str_c(
...
p_coum_3 <- cou_msa_df %>% 
  ...
  scale_fill_manual(
    values = nccr_colors_1[c(8,6,5,4,2)],
