...
edu_chart_1 <- edu_main %>% 
  ...
  labs(
    ...
    subtitle="Niveau de formation atteint le plus élevé") +
  scale_fill_nccr_categorical(order = c(3,4,12))
...
edu_chart2 <- edu_all_genre %>% 
  ...
  geom_text(data=. %>% 
            ...
            #fontface = "bold",
            position=position_nudge(0.2), hjust=0, 
            show.legend=FALSE, size = 3) +
  coord_cartesian(xlim = c(2016, 2026.4)) 


edu_chart2_a <- edu_chart2 +
  labs(
    title = str_c("Les <span style='color:#8E5A6F;'>femmes</span> ",
    "migrantes plus qualifiées que les <span style='color:#5DA6AD;'>hommes"),
...
edu3_pays_df_sub <- edu3_pays_df %>% 
  filter(
    year == 2024 & 
    type %in% c("Allemagne", "France", "Italie", "Autriche", "Autre OCDE", 
                "Portugal", "Espagne", "Autre UE/AELE", "Autre Europe", 
                "Autre OCDE",
                "Asie", 
...
edu_chart3 <- edu3_pays_df %>% 
  ...
  geom_point(data = edu3_pays_df %>% filter(year == 2024), aes(color = color, size = n, group = year)) + 
    geom_point(data = edu3_pays_df %>% filter(year == 2018), aes(color = color, size = n, group = year), shape = 15) + 
  geom_segment(
    data = edu3_pays_w,
  ...
  geom_richtext(
    ...
    aes(label = type, x = type, y = share_w,
        angle = angle,
        color = color),
    fill = NA, label.color = NA,# remove background and outline,
    size = 3.8,
  ...
  #          color = "#8c8c8c", size = 4,
  #          hjust = 0,
  #          label = "Part du total avec une formation tertaire en 2024") +
  labs(
    title = "Différences par pays d’origine",
    subtitle = "Les plus et les moins qualifiés, % avec formation  tertiaire en 2024"
  ) + 
