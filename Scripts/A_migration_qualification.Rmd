...
edu_chart_1 <- edu_main %>% 
  ...
  scale_y_continuous(expand = c(0,0), name = "", 
                     labels = function(x) paste0(x, "%")) +
  labs(
    title = str_c("Forte progression de la migration ", 
                  "<span style='color:#62836A'>hautement qualifiée</span>"),
    subtitle="Niveau de formation atteint le plus élevé") +
  scale_fill_nccr_categorical(order = c(5,6,12))
...
edu_chart2 <- edu_all_genre %>% 
  ...
  scale_y_continuous(expand = c(0.015,0.015), name = "", 
                     ...
                     position = "right") +
  scale_colour_manual(
    values=c(nccr_bi_colors[c(2,4)], "#898989")) +
...
edu_chart2_a <- edu_chart2 +
  labs(
    title = str_c("Les <span style='color:",
                  nccr_bi_colors[2], 
                  ";'>femmes</span> ",
    "migrantes plus qualifiées que les <span style='color:", 
    nccr_bi_colors[4], ";'>hommes"),
    subtitle = "Part avec formation tertiaire, selon le genre")
...
arrow_txt <-
  tibble(
    x1 = annot_sub$type-c(1,2),
    x2 = annot_sub$type,
    y1 = annot_sub$share_w-c(3,6),
...
edu_chart3 <- edu3_pays_df %>% 
  ...
  # geom_hline(yintercept = share_tot_3$share_w[2],
  #            linewidth = 1, color = "#898989", alpha = 0.5,
  #            linetype = "dotted") +
  # geom_hline(yintercept = share_tot_3$share_w[1],
  #            linewidth = 1, linetype = "dotted", 
  #            color = "#898989", alpha = 0.5) +
  geom_point(data = edu3_pays_df %>% 
             filter(year == 2024), 
             aes(color = color, size = n, group = year),
             alpha = 1) + 
    geom_point(
  ...
  geom_segment(
    ...
    aes(x=type, xend=type, y=`2018`, yend=`2024`,
        color = color, alpha = 0.4), size = 1, show.legend = F) +
  ...
  theme(
    ...
    legend.title = element_text(
      size = 10, color = mms_legend_base_col, 
  ...
  geom_curve(
    ...
    color = nccr_bi_colors[13], curvature = 0.2, size = .3) +
  geom_richtext(
    data = arrow_txt, 
  ...
  labs(
    title = "Plus de diplômés du tertiaire chez les ressortissants d’Asie et d’Europe de l’Est",
    subtitle = "Différence entre 2018 et 2024 du taux de formation tertiaire selon le pays d’origine"
  ) + 
...
svglite::svglite(
  filename = here("Output", "charts", "A_education.svg"),
  height = 11.7,
  width = 8.3,
  fix_text_size = T,
  bg = pw_bg_color
