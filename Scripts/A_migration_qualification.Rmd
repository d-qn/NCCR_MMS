...
edu_chart_1 <- edu_main %>% 
  ...
  labs(
    title = str_c("Strong increase in ", 
                  "<span style='color:#62836A'>highly skilled</span> migration"),
...
edu_chart2 <- edu_all_genre %>% 
  ...
  scale_y_continuous(expand = c(0.015,0.015), name = "", 
                     ...
                     position = "right") +
  scale_colour_manual(
    values=c(nccr_bi_colors[c(3,13,1)])) +
...
patchwork
# If that works, try ggsave with ragg
ggsave(
  here("Output", "charts", "A_education.svg"),
  plot = patchwork,
...
CairoPDF(here("Output", "charts", "A_education.pdf"),
    height = 11.7,
    width = 8.3,
    bg = pw_bg_color)
