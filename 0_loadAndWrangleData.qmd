...
edu_chart1 <- edu_main %>% 
  ...
  labs(
       ...
       subtitle="Part en % des personnes hautement qualifiées (niveau de formation tertiaire) parmi les répondant-e-s à l’enquête
")
...
edu_chart2 <- edu_all_genre %>% 
  ...
  geom_line(aes(x = year, y = share_w, group = type,
    color = type, size = n), lineend="round") +
  ...
  scale_y_continuous(expand = c(0,0), name = "", 
                     labels = function(x) paste0(x, "%")) +
  scale_colour_manual(values=c("#262626", "#7725eb", "#469b88")) +
  geom_text(data=. %>% 
              arrange(desc(year)) %>% 
              group_by(type) %>% 
              slice(1), 
