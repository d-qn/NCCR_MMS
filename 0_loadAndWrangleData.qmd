...
mig_chart1 <- migm_df %>% 
  ...
  theme(legend.position = c(0.98, 0.15),
        legend.title = element_text(color = "lightgrey", face = "bold", hjust = 0.5, size = 10),
...
mig_chart2 <- ggplot(mign_sub) + 
  geom_rect(aes(ymin = ymin, 
                ymax = ymax, xmin = xmin, 
                xmax = xmax, fill = type), 
            colour = "white", size = 0.5, alpha = 0.95) +
  theme_ipsum_rc(
    grid="X", axis="x", 
    grid_col = "black",
    base_size = rc_base_size,
  ...
  scale_fill_manual(values = c( "#3f2d54", "#c9d175")) +
...
value_labels <- mign_sub %>% 
  ...
  mutate(
    ...
    hjust = ifelse(type == "mar_share", 1.05, -0.25)
  ) 
...
mig_chart2_a <- mig_chart2 + 
  ...
  geom_richtext(
    ...
    fill = NA, label.color = NA,# remove background and outline,
    vjust = 1, size = 5, alpha = 0.9, colour = "white",
    family = font_rc_light
  ) +
  annotate(
    geom = "richtext",
    x=92,y = 0.5, angle = -90, label = "Seul",
    fill = NA, label.color = NA,# remove background and outline,
    size = 17,
  ...
  labs(
    title = "Les personnes des pays voisins migrent plus souvent seuls",
    subtitle = "Part des personnes en couple ou seules lors de leur migration, par pays"
  )
