...
migy2gr <- tibble(
  ...
  mutate(
    migg = ifelse(migg >= 2020, "2020-2022", str_c(migg,"-", migg+1)) %>% as.factor()
  )
...
migm_df %>% 
  ...
  geom_area(aes(fill = group), alpha = 0.9, show.legend = FALSE) + 
  geom_line(data = migm_df %>% filter(group == "Marié"),
            aes(x = migg, y = value, group = group, size = tot),
            lineend = "round", color = "white", alpha = 0.6
            ) +
  theme_ipsum_rc(grid="XY", axis="xy", 
                 grid_col = "black",
                 base_size = rc_base_size,
