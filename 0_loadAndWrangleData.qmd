...
```{r}
#| label: font embedding for svg setup
...
```{r}
#| label: some general graphics settings
...
edu_chart2 <- edu_all_genre %>% 
  ...
  theme(
    ...
    plot.title = rc_txtbox_plot.title(),
    plot.subtitle = rc_txtbox_plot.subtitle(),
    legend.text = element_text(size = 8, color = legend_base_col), # Reduce legend text size
    legend.title = element_text(size = 10, color = legend_base_col, face = "bold", 
  ...
  guides(color = "none", alpha = "none",
         size = guide_legend(override.aes = list(color = legend_base_col))) +
  geom_text(data=. %>% 
              arrange(desc(year)) %>% 
              group_by(type) %>% 
              slice(1), 
...
edu_chart3 <- edu3_pays_df_sub %>% 
  ...
  guides(size = guide_legend(override.aes = list(color = legend_base_col))) 
...
mig_chart1 <- migm_df %>% 
  ...
  theme(
    ...
    legend.title = element_text(
          color = legend_base_col, 
          face = "bold", hjust = 0.5, size = 10),
    legend.text = element_text(color = legend_base_col, size = 8),
    legend.justification = c(1, 0.8),
    legend.key.size = unit(0.6, "lines"), # Reduce legend key size
    #plot.title.position = "plot",
    plot.title = rc_txtbox_plot.title(),
    plot.subtitle = rc_txtbox_plot.subtitle(),
    axis.text.x = element_text(color = rc_axis_tick_txt_color),
  ...
  guides(size = guide_legend(override.aes = list(color = "lightgrey")))  +
  # remove any scale expansion
  ...
  labs(
    title="De 59 à 77% des personnes en couple lors de leur migration",
    subtitle=str_c("Proportion de personnes <span style = 'color:", "#758bd1", ";'>",
    "en couple</span> lors de leur migration"))
...
migcw_chart_a <- migcww %>% 
  ...
  theme_ipsum_rc(
   ...
   # grid_col = "black",
    ...
    plot_title_size = rc_plot_title_size,
    subtitle_size = rc_subtitle_size,
    plot_margin = plot_margin #margin(15, 5, 10, 0)
  ) + 
  theme(
    legend.position = "top",
    ...
    plot.title = rc_txtbox_plot.title(nrow_txt = 2),
    plot.subtitle = rc_txtbox_plot.subtitle(nrow_txt = 2)
  ) +
...
#  coord_flip() +                 # Make bars horizontal
 theme_ipsum_rc(
  ...
  theme(
    ...
    axis.ticks.y = element_blank() , # Hide y-axis ticks
    plot.title = rc_txtbox_plot.title(2),
    plot.subtitle = rc_txtbox_plot.subtitle(2),
    axis.text.x = element_text(color = rc_axis_tick_txt_color)
  ) + 
  #scale_fill_ipsum()+
  scale_fill_manual(values = c(darken("#75b8d1", amount =.2), 
                               "#d18975", "#d1ab75", "#758bd1")) +
...
patchwork2 <- (mig_chart1_a /migcw_chart_a) +
   plot_layout(heights = c(2, 1.2)) | mig_chart2_a
  ...
  plot_annotation(
    ...
    theme = theme(
      ...
      plot.title = element_textbox_simple(
        size = 23,
        hjust = 0.5,
        colour = darken("#758bd1", amount = 0.15),
        face = "bold",
        family = "Merriweather",
        lineheight = 1.3,
        padding = pw_padding,
...
pimt_chart1 <- pimt_sub %>% 
  ggplot() +
  geom_segment(data = pimt_subw, 
            aes(male,
                xend = female,
                y = year, yend = year),
            color = col_theme,
            lwd = 2, alpha = 0.9, show.legend = F) +
  geom_point(
    aes(x = share_non, y = year, fill = sexe,
        size = n_y, color = sexe)) +
  scale_size_continuous(range = c(6,8), name = "n") +
  theme_ipsum_rc(
    grid="X", axis="X", 
    base_size = rc_base_size,
    plot_title_size = rc_plot_title_size,
  ...
  theme(
    ...
    axis.text.y = element_text(
      ...
      face = "bold", color = darken(rc_axis_tick_txt_color, amount = .2)),
    legend.title = element_text(color = legend_base_col, face = "bold", hjust = 0.5, size = 10),
    legend.text = element_text(color = legend_base_col, size = 8),
    legend.justification = c(1, 0.8),
    legend.key.size = unit(0.4, "lines"), # Reduce legend key size
    axis.text.x = element_text(color = rc_axis_tick_txt_color)
  ) + 
  scale_color_manual(values = c("#7725eb", "#469b88")) +
  ...
  guides(fill = "none", color = "none", #size = "none"
         size = guide_legend(
    override.aes = list(fill = "lightgrey", colour = "lightgrey"))
    ) +
  geom_curve(
    data = arrow_txt2, 
    aes(x = x1, y = y1, xend = x2, yend = y2),
    color = "lightgrey", curvature = 0.4, size = .3) +
  annotate(geom = "text", x = 14, y = 2021.5, 
           family = font_rc,
           color = "#8c8c8c", size = 3,
           hjust = 1,
           label = "Ecart hommes-femmes") 
...
pimit_chart1_a <- pimt_chart1 +
  geom_richtext(data = pimt_sub,
                ...
                aes(x = share_non, y = year, 
                    color = sexe,
                    family = font_rc_light,
                    label = paste0(prettyNum(share_non, digits = 3),
                                   " %")),
                size = 3,
                vjust = 0, nudge_y = 0.17
  ) +
  ...
  labs(
    ...
    subtitle = "% de personnes qui considèrent ne pas utiliser leurs compétences au travail, par sexe et par année"
  )
...
pimit_chart2 <- pimt_sub2 %>% 
  ...
  geom_line(color = col_theme, size = 1, alpha = 0.6) +
  geom_point(aes(size = n), color = darken(col_theme)) +
  ...
  labs(
    ...
    subtitle = "% de personnes estimant sous-utiliser leurs compétences selon l’année d’arrivée"
  )
...
#    legend.position = c(0.93, 0.19),
    plot.caption = element_textbox_simple(
      color = "darkgrey", margin = margin(-10, 0,0,0)),
    plot.title = rc_txtbox_plot.title(nrow_txt = 2),
    plot.subtitle = rc_txtbox_plot.subtitle(),
    axis.text.y = element_text(
      margin = margin(t = 0, r = -30, b = 0, l = 0, unit = "pt"),
...
pimt_chart3_a <- pimt_chart3 +
  ...
  labs(
    ...
    caption = str_c("La largeur des bâtons est proportionnelle à la taille de l’échantillon (n), total des sondés: ",
  pimt_sub3 %>% .$n_y %>% sum() %>% prettyNum(big.mark = " "))
  )
  ...
  plot_annotation(
    ...
    theme = theme(
      plot.title = element_textbox_simple(
        size = 23,
        hjust = 0.5,
        colour = darken(col_theme, amount = 0.3),
        face = "bold",
        family = "Merriweather",
        lineheight = 1.3,
        padding = pw_padding,
