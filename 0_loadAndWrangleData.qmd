...
mig_brks <- c(-Inf, 2010, 2015, 2019, Inf)

migm <- data_all %>% 
  mutate(
    mig = year - ifelse(A6 < 0, NA, A6),
    mar = ifelse(B9 == 1, T, F),
    mig_g = cut(mig, breaks = mig_brks)
  ) %>% 
  group_by(mig_g) %>% 
  summarise(
    n = n(),
    tot_weight = sum(weight, na.rm = T),
    mar = sum(weight[which(mar)], na.rm = T),
    mar_share = (mar / tot_weight) * 100
  ) %>% 
  ungroup 
