...
edu_chart1 <- edu_main %>% 
  ...
  labs(
    ...
    subtitle="Niveau de formation atteint le plus élevé parmi les répondant-e-s à l’enquête, en %")
...
edu_chart2 <- edu_all_genre %>% 
  ...
  # have y scale not start at 0, add unit % 
  scale_y_continuous(expand = c(0.007,0.012), name = "", 
                     labels = function(x) paste0(x, "%")) +
  scale_colour_manual(
...
edu_chart2_a <- edu_chart2 +
  labs(subtitle = "Part avec formation tertiaire selon le genre")
...
patchwork + 
  plot_annotation(title="La migration est de plus en plus hautement qualifiée",
                  theme = theme_ipsum_rc(plot_title_size = 25)) 
